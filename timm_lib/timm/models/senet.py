...
class SENet(nn.Module):
    ...
    def forward_mc_dropout(self, x, dropout_iterations=30):
        x = self.forward_features(x)
        predictions = torch.zeros((dropout_iterations, x.shape[0], 1000), device='cuda')  # 0, n_classes
        for i in range(dropout_iterations):
            output = self.forward_head(x)
            predictions[i, :, :] = torch.softmax(output, dim=1)
